# adopted from https://github.com/Meaquadddd/DPO-Shift/blob/main/scripts/dposhift_trainer.py
import inspect
import os
import random
import warnings
...
@dataclass
class DPOShiftConfig(TrainingArguments):
    r"""
    Initialize DPOShiftConfig.
    ...
    Args:
        ...
        dpo_lambda (`float`, defaults to 1.0):
            The f(\lambda) in DPO-Shift loss. Higher lambda means higher win rate and lower
        dpo_lambda_strategy (`str`, defaults to `"fixed"`):
            The strategy to update the lambda factor in DPO loss. Either `"fixed"`, `"decrease_linear"` or `"increase_linear"`.
        dpo_lambda_min (`float`, defaults to 0.5):
            The minimum lambda factor in DPO loss.
        dpo_lambda_max (`float`, defaults to 1.0):
            The maximum lambda factor in DPO loss.
    ...
    dpo_lambda_strategy: Literal["fixed", "decrease_linear", "increase_linear"] = (
        "fixed"
    )
    dpo_lambda_min: float = 0.5
    dpo_lambda_max: float = 1.0
...
class DPOShiftTrainer(Trainer):
    r"""
    Initialize DPOShiftTrainer.
    ...
    @_deprecate_arguments(
        ...
        custom_message="Deprecated positional argument(s) used in DPOShiftTrainer, please use the DPOShiftConfig to set these arguments instead.",
    )
    def __init__(
        self,
        ...
        args: Optional[DPOShiftConfig] = None,
        data_collator=None,
        label_pad_token_id: int = -100,
        ...
        if model_init_kwargs is not None:
            warnings.warn(
                "You passed `model_init_kwargs` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if args.model_init_kwargs is None:
            ...
            raise ValueError(
                "You passed model_init_kwargs to the DPOShiftTrainer/DPOShiftConfig, but your model is already instantiated."
            )
        else:
            ...
            if torch_dtype is not None:
                ...
                if torch_dtype != "auto" and not isinstance(torch_dtype, torch.dtype):
                    raise ValueError(
                        f"Invalid `torch_dtype` passed to the DPOShiftConfig. Expected a string with either `torch.dtype` or 'auto', but got {torch_dtype}."
                    )
        ...
        if ref_model_init_kwargs is not None:
            warnings.warn(
                "You passed `ref_model_init_kwargs` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if args.ref_model_init_kwargs is None:
            ...
            raise ValueError(
                "You passed ref_model_init_kwargs to the DPOShiftTrainer/DPOShiftConfig, but your ref_model is already instantiated."
            )
        else:
        ...
        if isinstance(model, str):
            warnings.warn(
                "You passed a model_id to the DPOShiftTrainer. This will automatically create an "
        ...
        if isinstance(ref_model, str):
            warnings.warn(
                "You passed a ref model_id to the DPOShiftTrainer. This will automatically create an "
                "`AutoModelForCausalLM`"
            )
        ...
        if force_use_ref_model:
            warnings.warn(
                "You passed `force_use_ref_model` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if not is_peft_available() and peft_config is not None:
            ...
            if ref_model is not None and not args.force_use_ref_model:
                raise ValueError(
                    ...
                    " model. Please pass `ref_model=None` in case you want to train PEFT adapters, or pass a ref_model with `force_use_ref_model=True` in DPOShiftTrainer's init."
        ...
        if generate_during_eval:
            warnings.warn(
                "You passed `generate_during_eval` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if is_encoder_decoder is not None:
            warnings.warn(
                "You passed `is_encoder_decoder` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if model is not None:
            ...
            raise ValueError(
                "When no model is provided, you need to pass the parameter is_encoder_decoder to the DPOShiftTrainer/DPOShiftConfig."
            )
        else:
        ...
        if model_adapter_name is not None:
            warnings.warn(
                "You passed `model_adapter_name` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if ref_adapter_name is not None:
            warnings.warn(
                "You passed `ref_adapter_name` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if reference_free:
            warnings.warn(
                "You passed `reference_free` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if precompute_ref_log_probs:
            warnings.warn(
                "You passed `precompute_ref_log_probs` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if max_length is not None:
            warnings.warn(
                "You passed `max_length` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.max_length = max_length
        if args.max_length is None:
            warnings.warn(
                "`max_length` is not set in the DPOShiftConfig's init"
        ...
        if max_prompt_length is not None:
            warnings.warn(
                "You passed `max_prompt_length` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.max_prompt_length = max_prompt_length
        if args.max_prompt_length is None:
            warnings.warn(
                "`max_prompt_length` is not set in the DPOShiftConfig's init"
        ...
        if max_target_length is not None:
            warnings.warn(
                "You passed `max_target_length` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.max_target_length = max_target_length
        if args.max_target_length is None and self.is_encoder_decoder:
            warnings.warn(
                "When using an encoder decoder architecture, you should set `max_target_length` in the DPOShiftConfig's init"
                " it will default to `128` by default, but you should do it yourself in the future.",
                UserWarning,
            )
        ...
        if label_pad_token_id != -100:
            warnings.warn(
                "You passed `label_pad_token_id` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if not disable_dropout:
            warnings.warn(
                "You passed `disable_dropout` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if padding_value is not None:
            warnings.warn(
                "You passed `padding_value` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if truncation_mode != "keep_end":
            warnings.warn(
                "You passed `truncation_mode` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if loss_type != "sigmoid":
            warnings.warn(
                "You passed `loss_type` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.loss_type = loss_type
        if label_smoothing != 0:
            warnings.warn(
                "You passed `label_smoothing` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
        ...
        if args.loss_type == "kto_pair":
            raise ValueError(
                "Support for kto_pair has been removed in DPOShiftTrainer. Please use KTOTrainer."
            )

        if beta != 0.1:
            warnings.warn(
                "You passed `beta` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.beta = beta
        self.beta = args.beta
        ...
        if dataset_num_proc is not None:
            warnings.warn(
                "You passed `dataset_num_proc` to the DPOShiftTrainer, the value you passed will override the one in the `DPOShiftConfig`."
            )
            args.dataset_num_proc = dataset_num_proc
        self.dataset_num_proc = args.dataset_num_proc

        self.dpo_lambda = args.dpo_lambda
        self.dpo_lambda_min = args.dpo_lambda_min
        self.dpo_lambda_max = args.dpo_lambda_max
        self.dpo_lambda_strategy = args.dpo_lambda_strategy
        self.data_path = data_path
        train_dataset_path = f"{self.data_path}/train_dataset.pt"
        eval_dataset_path = f"{self.data_path}/eval_dataset.pt"

        # Compute that only on the main process for faster data processing.
        # see: https://github.com/huggingface/trl/pull/1255

        # save the train and eval dataset so that we don't have to tokenize them again
        if not (
            os.path.exists(train_dataset_path) and os.path.exists(eval_dataset_path)
        ):
            ...
            torch.save(eval_dataset, eval_dataset_path)
        else:
            pass
    ...
    def dpo_loss(
        ...
        l_min = torch.tensor(self.dpo_lambda_min).to(self.accelerator.device)
        l_max = torch.tensor(self.dpo_lambda_max).to(self.accelerator.device)

        if self.lr_scheduler is not None:
            current_step = torch.tensor(
                self.lr_scheduler.__dict__["_step_count"] - 1
            ).to(self.accelerator.device)

            if self.dpo_lambda_strategy == "increase_linear":
                self.dpo_lambda = l_min + (l_max - l_min) * (
                    current_step / self.max_steps
                )
            elif self.dpo_lambda_strategy == "decrease_linear":
                self.dpo_lambda = l_max - (l_max - l_min) * (
                    current_step / self.max_steps
                )
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            if self.reference_free:
                ...
                ref_logratios = (
                    reference_chosen_logps - self.dpo_lambda * reference_rejected_logps
                )
    ...
    def get_batch_loss_metrics(
        ...
        metrics[f"{prefix}dpo_lambda"] = self.dpo_lambda
    ...
    def prediction_step(
        ...
        # import os
        # if os.environ.get('LOCAL_RANK') == '0':  # 判断是否为主进程
        #     breakpoint()

        # if os.environ.get('LOCAL_RANK') == '1':  # 判断是否为第二个进程
        #     # breakpoint()
        #     print(self._stored_metrics["eval"])
        # force log the metrics
